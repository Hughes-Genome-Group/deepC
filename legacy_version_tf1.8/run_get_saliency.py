...
# NOTE 0 based indexed bed like format --> eneed to implemenet switch between 1 and 0 based
...
def placeholder_inputs(batch_size, dtype):
    ...
    if dtype == 'bool':
        seqs_placeholder = tf.placeholder(tf.bool, [None, FLAGS.bp_context, 4], name='seqs')
        labels_placeholder = tf.placeholder(tf.uint8, shape=[None, FLAGS.num_classes], name='labels')
    if dtype == 'uint8':
        seqs_placeholder = tf.placeholder(tf.uint8, [None, FLAGS.bp_context, 4], name='seqs')
        labels_placeholder = tf.placeholder(tf.uint8, shape=[None, FLAGS.num_classes], name='labels')
    if dtype == 'float32':
        ...
        seqs_placeholder = tf.placeholder(tf.int32, [None, FLAGS.bp_context, 4], name='seqs')
        labels_placeholder = tf.placeholder(tf.int32, shape=[None, FLAGS.num_classes], name='labels')
...
def get_hot_coded_seq(sequence):
    ...
    # initialise empty
    hotsequence = np.zeros((len(sequence),4), dtype='float')
