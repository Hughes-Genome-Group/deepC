...
setwd("~/Data/machine_learning/deepC_write_tutorials_offline/tutorials/")

```

This tutorial demonstrates how to format HiC data for deepC training. The tutorial walks through formating a single chromosomes. In practice we prepare each chromosome separately and concatinate all chromosome files in the end to a single file for training.
...
genome.sizes <- read.table('hg19_chrom_sizes.txt')  # link to chromosome sizes file (ucsc style)
...
Set the chromosome and read in the HiC data. The skeleton transformation is done chromosome wise and for memory friendliness we recommend doing this separate for each chromosome and concatinating the output into a single file.

We support two HiC data formats, both are sparse matrix formats: 
...
Both formats list the left most base of a bin sized intervall as identifier for the bin position.

We first read in the HiC data, inspect the object and create a binned genome template for filling.
...
# The HiC data is stored in a structures list storing the bin size start position and stat id as well as the coordinates with bin associations
str(hic)
...
Next we convert the HiC interactions first into deepC format with a vvertical zig-zag pole associated with the center of each bp_context sized sequence window.
Second we transform the interaction values into the HiC skeleton by applying the pyramid shaped percentile binning. The data are percentile normalized into unequal bins from low to high values in the following scheme: 2x20% 4x10% 4x5% percentiles.
...
# There is a memory friendly version of the function which relies on two perl helper scripts but takes a tad longer
tdf <- getZigZagWindowInteractionsPerl(hic, binned.genome, window.size, bin.size, 
                                                             query.pl= "../deepC/helper_for_preprocessing_and_analysis/match_query_table.pl")
# tdf <- getZigZagWindowInteractionsPerlMemoryFriendlyFullPerl(hic, binned.genome, window.size, bin.size, 
...
# padd with median of respective interaction distance for calculating the median on edge cases
...
```{r, example_plot, fig.height=3, fig.width=9}
  ...
  p <- ggplot(tpdf, aes(y=bin, x=pos, fill=value, group = polyid)) + geom_polygon() + scale_fill_gradientn(colours = brewer.pal(9, 'YlOrRd'))
...
```{r}
  ...
  write.table(bdf, file = paste0("coords_and_hic_skeleton_for_deepC_5kb_", chrom,".bed"), col.names = F, row.names = F, quote = F, sep = "\t")
  
```


This will store a file per chromosome, which we then concatinate into a single file for all chromosomes and supply this to the deepC training routines. Find an example under *formatted_data_links*.
